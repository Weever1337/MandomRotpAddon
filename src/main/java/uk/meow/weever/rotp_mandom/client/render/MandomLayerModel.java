...
public class MandomLayerModel<T extends LivingEntity, M extends BipedModel<T>> extends PlayerModel<T> {
    ...
    public void setupAnim(LivingEntity entity, float limbSwing, float limbSwingAmount, float ticks, float yRot, float xRot) {
        m_head.xRot = -0.0436332312999F * (1 + MathHelper.cos(ticks * .05F));
        m_body.xRot = -0.00471238898038F * (1 + MathHelper.cos(ticks * .05F));
        m_body.yRot = -0.00541052068118F * (1 + MathHelper.cos(ticks * .05F));
        m_body.zRot = -0.00506145483078F * (1 + MathHelper.cos(ticks * .05F));
        shoulder.yRot = -0.0436332312999F * (1 + MathHelper.cos(ticks * .05F));
    }

    @Override
    public void renderToBuffer(MatrixStack matrixStack, IVertexBuilder buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        Mandom.render(matrixStack, buffer, packedLight, packedOverlay, red, green, blue, alpha);
    }
